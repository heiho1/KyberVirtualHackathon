// derived from https://github.com/Uniswap/uniswap-frontend/blob/beta/src/contexts/Balances.js
import React, {
  createContext,
  useContext,
  useReducer,
...
import { getEtherBalance, getTokenBalance } from '../../utils/tokens';
import { isAddress } from '../../utils/contracts';
import { useBlockNumber } from './Application';
import { useTokenDetails, useAllTokenDetails } from './Tokens';
import { safeAccess, getUSDPrice } from '../zap_uniswap';
      ...
      if (chainId && library && account) {
        ...
        Promise.all(
          allTokensWithAnExchange.map(async tokenAddress => {
            ...
            const { exchangeAddress } = allTokens[tokenAddress];
            const { value: existingValue } =
...
export function useETHPriceInUSD() {
  ...
  const daiReserveETH = useAddressBalance(daiExchangeAddress, 'ETH');
  const daiReserveToken = useAddressBalance(
    daiExchangeAddress,
    daiTokenAddress
  );
  const usdcReserveETH = useAddressBalance(usdcExchangeAddress, 'ETH');
  const usdcReserveToken = useAddressBalance(
    usdcExchangeAddress,
    usdcTokenAddress
  );
  const tusdReserveETH = useAddressBalance(tusdExchangeAddress, 'ETH');
  const tusdReserveToken = useAddressBalance(
    tusdExchangeAddress,
    tusdTokenAddress
  );
